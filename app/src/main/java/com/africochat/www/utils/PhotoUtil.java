...
final public class PhotoUtil {
        ...
        if (takePictureIntent.resolveActivity(context.getPackageManager()) != null) {
            ...
            if (photoFile != null) {
                Uri photoURI = FileProvider.getUriForFile(context,
                        "com.africochat.www.fileprovider",
                        photoFile);
    ...
    public interface CropPhotoCallback {
        void onCropPhotoDone(boolean isSuccessful, Uri photoUri);
    }

    /**
     * Crop photo after taking it from gallery or camera
     */
    public static void cropPhoto(Context context, String currentPhotoPath, Fragment fragment, CropPhotoCallback cropPhotoCallback) {
        if (currentPhotoPath != null) {
            File f = new File(currentPhotoPath);
            Uri contentUri = Uri.fromFile(f);
            // start cropping activity for pre-acquired image saved on the device
            if (fragment != null) {
                cropPhotoCallback.onCropPhotoDone(true,contentUri);
            } else {
                cropPhotoCallback.onCropPhotoDone(false,null);
            }
        }
    }
